...
class Ningmeng(Sprite):
    def __init__(self, ai_settings, screen):
        super().__init__()
        self.screen = screen
        self.ai_settings = ai_settings
        
        self.image = pygame.image.load('images/ningmeng.bmp')
    ...
    def update(self,screen,ai_settings,laoba,stats,shou,shou2,num_baba,num_materials,hands,hands2):
        ...
        self.y += self.ai_settings.ningmeng_speed_factor
        self.rect.y = self.y
        
        if pygame.sprite.spritecollide(self,hands,False):
            num_materials.append('ningmeng')
            pygame.mixer.pre_init()
            pygame.mixer.init()
            b = pygame.mixer.Sound('music/加柠檬.wav')
            b.play()
